...
longitudeMin = min(longitudeParArret) -0.0005

lattitudeMax = max(lattitudeParArret) +0.065
lattitudeMin = min(lattitudeParArret) -0.0009

diffLong = (longitudeMax - longitudeMin) * 2
diffLat = (lattitudeMax - lattitudeMin) #* 2
...
class Map:
    ...
    def draw_line_betwenn_points(self, point_1, point_2, color: str = "steel blue", width: int = 1) -> None:
        ...
        return line
    def draw_line_between_point_and_pred(self, arret, pred, color: str = "steel blue", width: int = 1):
    ...
    while len(l_ouverte) != 0:
        ...
        for nomArret in voisins:
            ...
            if arrets_traverses[nomArret] <= 9:
                window.draw_line_betwenn_points(arret_courant.nom, nouvel_arret.nom,
                                                  couleursArrets(arrets_traverses[nomArret]))
...
def floyd_warshallGraphique(arret_dep, arret_arriv, window):
    ...
    for k in range(len(noms_arrets)):
        ...
        # Boucle de calcul de Mk
        for i in colonnes:    
            window.draw_line_betwenn_points(nom(k), nom(i))
            for j in lignes:
...
def affichageGraphiqueAlgorithme():
    ...
    texteAlgoDispo = gr.Text(gr.Point(450, 150), "B ou b = Bellman; D ou d = Djikstra; F ou f = Floyd-Warshall; Q ou q = pour quitter l'application")
